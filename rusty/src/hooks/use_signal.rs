...
mod tests {
    ...
    fn test_two_views_in_one_session_share_a_signal() {
        ...
        let subscription = {
            let mut ctx =
                BuildContext::with_services(&mut receiver_store, None, uuid::Uuid::nil(), Arc::clone(&services));
        ...
        {
            let mut ctx =
                BuildContext::with_services(&mut sender_store, None, uuid::Uuid::nil(), Arc::clone(&services));
            let signal = use_signal::<String, ()>(&mut ctx, "refresh", SignalScope::Session);
    ...
    fn test_session_and_server_scopes_are_separate_signals() {
        ...
        {
            let mut ctx = BuildContext::with_services(&mut store, None, uuid::Uuid::nil(), Arc::clone(&services));
    ...
    fn test_two_sessions_do_not_share_session_scoped_signals() {
        ...
        let _sub_a = {
            let mut ctx =
                BuildContext::with_services(&mut store_a, None, uuid::Uuid::nil(), Arc::clone(&services_a));
        ...
        {
            let mut ctx =
                BuildContext::with_services(&mut store_b, None, uuid::Uuid::nil(), Arc::clone(&services_b));
            let signal = use_signal::<i32, ()>(&mut ctx, "tick", SignalScope::Session);
            signal.send(1);
        }
        ...
        {
            let mut ctx = BuildContext::with_services(&mut store_b, None, uuid::Uuid::nil(), services_b);
            let signal = use_signal::<i32, ()>(&mut ctx, "broadcast", SignalScope::Server);
            signal.send(1);
        }
    ...
    fn test_receiver_id_is_stable_across_rebuilds() {
        ...
        let second = {
            let mut ctx = BuildContext::with_services(&mut store, None, uuid::Uuid::nil(), services);
            let _signal = use_signal::<i32, i32>(&mut ctx, "s", SignalScope::Session);
            use_receiver_id(&mut ctx)
        };
