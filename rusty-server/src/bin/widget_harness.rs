...
mod tests {
    ...
    /// registers these services. `use_query` panics without `QueryService`, and
    /// `use_download_stream` / `use_download_bytes` panic without `DownloadService` —
    /// this list must track `create_session`, or adding a hook to a harness app fails
    /// the exhaustiveness test below instead of the app.
